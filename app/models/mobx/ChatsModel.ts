...
class ChatsModel extends FirestoreModel<IChat> {
  ...
  sendPushNotification(fcmToken: string, chat: string, message: IMessagePayload) {
    ...
    fetch('https://fcm.googleapis.com/fcm/send', requestOptions)
      ...
      .then(result => console.log('https://fcm.googleapis.com/fcm/send', result))
      .catch(error => console.log('https://fcm.googleapis.com/fcm/send error', error));
  }
  async sendMessage(to: string, message: IMessagePayload) {
    ...
    const companionRef = target.members.find((user) => user.id != AccountModel.id)
